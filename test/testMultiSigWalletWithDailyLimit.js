...
describe('MultiSigWalletWithDailyLimit', async () => {
    const isOVM = await isOptimisticEthereum(web3);

    // This test suite is almost entirely focused around sending native ETH value, which is not
    // supported on the OVM. If we're running on the OVM, we just skip it.
    (isOVM ? contract.skip : contract)('MultiSigWalletWithDailyLimit', (accounts) => {
